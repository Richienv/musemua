    ...
    <Popover open={isOpen} onOpenChange={setIsOpen} modal={true}>
      <PopoverTrigger asChild>
        <Button 
          variant="ghost" 
          size="icon" 
          className="relative w-14 h-14 hover:bg-gray-100 transition-colors" 
          onClick={() => {
            setIsOpen(true);
            fetchNotifications();
          }}
        >
          ...
          {unreadCount > 0 && (
            <span className="absolute top-2 right-3 h-3.5 w-3.5 bg-red-500 rounded-full"></span>
          )}
        </Button>
      </PopoverTrigger>
      <PopoverContent 
        ...
        <div className="bg-white rounded-lg shadow-lg overflow-hidden">
          ...
          <div className="max-h-[calc(100vh-200px)] overflow-y-auto">
            {notifications.length === 0 ? (
              <p className="text-center py-6 text-gray-500">No new notifications</p>
            ) : (
              notifications.map((notification) => (
                <div 
                  key={notification.id} 
                  className={`p-4 border-b border-gray-200 transition-colors duration-150 ease-in-out cursor-pointer
